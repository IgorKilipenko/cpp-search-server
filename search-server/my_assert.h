...
class AssertionFailed final {
   public:
    AssertionFailed(std::string message, std::string file, std::string func, unsigned line)
        : message_{std::move(message)}, file_{std::move(file)}, function_{std::move(func)}, line_(line) {}
...
#define assert(expression) (void)((!!(expression)) || (::detail::AssertImpl(#expression, __FILE__, __FUNCTION__, static_cast<unsigned>(__LINE__)), 0))
